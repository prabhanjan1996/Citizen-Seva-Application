...
class QuizCreateView(CreateView):
    ...
    labels = ('Title', 'Department', 'Description')
...
def question_add(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('teachers:question_add', quiz.pk, complaint.pk)
    else:
        form = QuestionForm()
...
@login_required
@teacher_required
def question_change(request, quiz_pk, complaint_pk):
    ...
    question = get_object_or_404(Complaint, pk=complaint_pk, quiz=quiz)

    AnswerFormSet = inlineformset_factory(
        Complaint,  # parent model
...
def question_add1(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            question.quiz = quiz
            question.save()
            messages.success(request, 'Your Complaint Has Been Registered Sussessfully')
            return redirect('teachers:quiz_change_list')
    else:
        form = QuestionForm()
...
def pdf(request, pk):
    # Generated by Django 2.0.1 on 20def pdf(request, pk):
    response = HttpResponse(content_type='application/pdf')
    response['Content-Disposition'] = 'inline; filename="mypdf.pdf"'
    complain = get_object_or_404(Quiz, id=pk)
    buffer = BytesIO()
    ...
    p.drawCentredString(300, 750, "Complaint About")

    # p.setFillColorRGB(0, 0, 9)
    p.setFont("Helvetica", 24)
    p.drawCentredString(125, 700, "Title : " + complain.name)
    p.setFont("Helvetica", 20, )
    p.drawCentredString(105, 670, "Complaint By : " + complain.owner.username)

    p.setFontSize(18)
    p.drawCentredString(47, 600, "Details : ")
    p.setFontSize(16)
    p.drawCentredString(320, 560, " " + complain.Description)

    p.setFontSize(16)
    p.drawCentredString(450, 80, "Signature: ")
    p.drawCentredString(80, 80, "Place: " )
