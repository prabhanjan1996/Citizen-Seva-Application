...
def edit_state(request, id):
    users = Quiz.objects.get(id=id)
    context = {'users': users}
    return render(request, 'classroom/students/edit.html', context)


def update_state(request, id):
    i = Quiz.objects.get(id=id)
    i.Description = request.POST['Description']

    i.save()
    return HttpResponseRedirect(reverse('students:quiz_list'))
